...
class PolyTrans_Translation_Notifications
{
    ...
    {
        error_log("[polytrans] transition_post_status: post {$post->ID} from $old_status to $new_status");
        ...
        if (!$author) {
            error_log("[polytrans] No author found for original post $original_post_id (translation {$post->ID})");
            return;
        }
        ...
        error_log("[polytrans] Sending review notification to author {$author->user_email} for post {$post->ID} (original $original_post_id)");
        error_log("[polytrans] Email subject: $subject");
        error_log("[polytrans] Email body: $body");
        ...
        error_log("[polytrans] Marked as notified for post {$post->ID}");
    }
    ...
    public function send_reviewer_notification($post_id, $reviewer_id, $lang)
    {
        ...
        if (!$reviewer) {
            error_log("[polytrans] No reviewer found with ID $reviewer_id");
            return false;
        }
        ...
        error_log("[polytrans] Sending reviewer notification to {$reviewer->user_email} for post $post_id in language $lang");
