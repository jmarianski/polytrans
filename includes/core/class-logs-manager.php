...
class PolyTrans_Logs_Manager
{
    ...
    public static function create_logs_table()
    {
        ...
        // Using log_time instead of timestamp to avoid conflicts with reserved words
    ...
    private static function check_and_adapt_table_structure($table_name)
    {
        ...
        // Check if the table has either timestamp or log_time column
        $has_time_column = in_array('timestamp', $existing_columns) || in_array('log_time', $existing_columns);
        if (!$has_time_column) {
            // Try to add log_time column
            $wpdb->query("ALTER TABLE `$table_name` ADD COLUMN `log_time` datetime NOT NULL");
            error_log("[polytrans] Added missing log_time column to logs table");
        }
    ...
    public static function get_logs($args = [])
    {
        ...
        if (!empty($args['date_from'])) {
            ...
            $sql .= $wpdb->prepare(" AND timestamp >= %s", $date_from);
            $count_sql .= $wpdb->prepare(" AND timestamp >= %s", $date_from);
        }
        
        if (!empty($args['date_to'])) {
            $date_to = sanitize_text_field($args['date_to']);
            $sql .= $wpdb->prepare(" AND timestamp <= %s", $date_to);
            $count_sql .= $wpdb->prepare(" AND timestamp <= %s", $date_to);
        }
        
        // Order
        $orderby = in_array($args['orderby'], ['id', 'timestamp', 'level', 'source', 'user_id']) ? $args['orderby'] : 'timestamp';
    ...
    public static function clear_old_logs($days = 30)
    {
        ...
        // Get the date column name (timestamp or log_time)
        $columns = self::get_table_columns($table_name);
        $date_column = in_array('log_time', $columns) ? 'log_time' : 
                      (in_array('timestamp', $columns) ? 'timestamp' : null);
    ...
    public static function admin_logs_page()
    {
        ...
        <div class="wrap">
            ...
            <table class="wp-list-table widefat fixed striped">
                <thead>
                    <tr>
                        <th scope="col" class="manage-column column-timestamp"><?php esc_html_e('Time', 'polytrans'); ?></th>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($logs_data['logs'] as $log): ?>
                            <tr>
                                <td>
                                    <?php echo esc_html(mysql2date('Y-m-d H:i:s', $log->timestamp)); ?>
                                </td>
                                <td>
                                    <?php
                                    $level_class = '';
    ...
    public static function log($message, $level = 'info', $context = [])
    {
        ...
        try {
            ...
            $column_mappings = [
                'timestamp' => ['timestamp', 'log_time', 'created_at', 'date'],
            ...
            $time_column = self::find_first_match($existing_columns, $column_mappings['timestamp']);
