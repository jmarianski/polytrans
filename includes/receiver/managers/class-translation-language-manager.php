...
class PolyTrans_Translation_Language_Manager
{
    ...
    private function assign_language($new_post_id, $target_language)
    {
        if (function_exists('pll_set_post_language')) {
            ...
            error_log("[polytrans] Set language for post $new_post_id to $target_language");
        } else {
            error_log("[polytrans] Polylang not available, skipping language assignment for post $new_post_id");
        }
    }
    ...
    private function setup_translation_relationship($new_post_id, $original_post_id, $target_language)
    {
        if (function_exists('pll_save_post_translations')) {
            ...
            error_log("[polytrans] Set up translation relationship: post $new_post_id as $target_language translation of $original_post_id");
        }
    ...
    private function configure_post_status($new_post_id, $target_language)
    {
        ...
        error_log("[polytrans] Set post status for $new_post_id to $desired_status (language: $target_language)");
    }
}
